...
//You need to change these to template classes and implement the functions for these classes
    ...
    {
    public:
        node_iterator<Item>(node<Item>* initial = NULL) {current = initial;}
...
class const_node_iterator
    ...
    {
    public:
        const_node_iterator<Item>(const node<Item>* initial = NULL)	{current = initial;}
        const Item& operator *( ) const	{return current->data();}       // Dereference, gets data of current node
        const_node_iterator<Item>& operator ++( )	{current = current->link();}   // Prefix ++
        const_node_iterator<Item> operator ++(int)	
		{
			node_iterator<Item> original(current);
			current = current->link();
		}  // Postfix ++
        bool operator ==(const const_node_iterator<Item> other) const	{return (current == other.current);}
        bool operator !=(const const_node_iterator<Item> other) const	{return (current != other.current);}
    private:
